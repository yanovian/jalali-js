...
| **jalali-js**                                                                            | Conversion core + bindings + UI  | Yes       | Yes (plugin interface) | Explicit `CalendarDate`/`CalendarDateTime`/`ZonedCalendarDateTime` tiers | Yes               | React and Vue      | Headless primitives, styled `DatePicker` on top |
