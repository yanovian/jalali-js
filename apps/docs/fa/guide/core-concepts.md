...
[مقدار نمایشی در برابر مقدار ذخیره‌سازی](/fa/guide/display-vs-storage)). `system: 'gregorian'` روی
...
گزینه `precision` در `createCalendar()` انتخاب کنید؛ overloadهای TypeScript برای هر دقت،
...
می‌گیرد و آن را رندر می‌کند؛ prop با نام `locale` در بایندینگ‌های React، Vue و Web Components
