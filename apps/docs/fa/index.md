...
hero:
  name: jalali-js
  text: کتابخانه تقویم جلالی با TypeScript بومی
  tagline: تاریخ جلالی (هجری شمسی) را نمایش دهید. مقدار میلادی را به‌صورت پیش‌فرض ذخیره کنید، بدون کار اضافه.
  actions:
    - theme: brand
      text: نسخه زنده
...
features:
  - title: نمایش جلالی، ذخیره میلادی
    details: هر مؤلفه و هر تابع تبدیل هسته، به‌صورت پیش‌فرض یک مقدار میلادی و مستقل از تقویم برمی‌گرداند؛ همان قرارداد یک <input type="date"> بومی. وقتی واقعاً نیاز دارید، با یک گزینه مقدار ذخیره‌شده جلالی را انتخاب کنید.
  ...
  - title: React، Vue، یا بدون فریم‌ورک
    details: ابتدایی‌های بدون ظاهر (ویژگی‌های data و scoped slot) برای کنترل کامل، به‌همراه DatePicker با ظاهر پیش‌فرض برای استفاده فوری. @jalali-js/web همان مؤلفه‌ها را به‌صورت Web Components می‌فرستد، تا صفحه بدون فریم‌ورک یا هر میزبانی بدون binding اختصاصی، همان انتخابگر و CSS قالب را بگیرد.
...
| [`@jalali-js/react`](https://www.npmjs.com/package/@jalali-js/react)       | رابط React                 |
| [`@jalali-js/vue`](https://www.npmjs.com/package/@jalali-js/vue)           | رابط Vue                   |
