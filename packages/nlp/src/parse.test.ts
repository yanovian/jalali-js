...
describe('parse: Finglish (fa-Latn)', () => {
  it('accepts common spelling variants for each relative term', () => {
    for (const word of ['emrooz', 'emruz', 'emrouz']) {
      expect(parse(word, 'fa-Latn')).toEqual({
        precision: 'date',
        system: 'jalali',
        year: 1403,
        month: 5,
    ...
    for (const word of ['dirooz', 'diruz', 'dirouz']) {
      expect(parse(word, 'fa-Latn')).toEqual({
        precision: 'date',
        system: 'jalali',
        year: 1403,
    ...
    expect(parse('farda', 'fa-Latn')).toEqual({
      precision: 'date',
      system: 'jalali',
      year: 1403,
      month: 5,
  ...
  it('reads "next week" variants', () => {
    for (const phrase of ['hafte ayande', 'hafteh ayande', 'hafte baad', 'hafte bad']) {
      expect(parse(phrase, 'fa-Latn')).toEqual({
        precision: 'date',
        system: 'jalali',
        year: 1403,
        month: 5,
  ...
  it('reads "<month> ayande", case-insensitively, in suffix order', () => {
    expect(parse('Farvardin ayande', 'fa-Latn')).toEqual({
      precision: 'date',
      system: 'jalali',
      year: 1404,
      month: 1,
  ...
  it('returns null for unrecognized Finglish input', () => {
    expect(parse('mikham beram', 'fa-Latn')).toBeNull();
  });
});
