...
1. **No visual-baseline branch exists yet.** `visual-baselines` and `visual-snapshots` are both
   absent from `origin`. Run `update-visual-baselines.yml` once, before or shortly after this
   release, so `e2e.yml` has a real baseline to diff against on the next PR.
