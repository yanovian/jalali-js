...
- [x] **Holiday name tip on hover and focus.** On days with `data-holiday`,
      ...
      `holidayName`) for the active locale. Show the tip as an overlay on
      the calendar (`data-jalali-calendar-tip`) on hover and keyboard focus
      only, without changing the calendar size. Do not open the tip on click
      or press, so a normal select still works. Include the name in the
      accessible name. Support more than one holiday on the same day. Wire
      React, Vue, and Web Calendar (and RangePicker day cells that mark
      holidays).
